...
export async function startStudio(options: StudioOptions): Promise<void> {
  ...
  // Load embedded asset manifest if available (generated by build:client)
  let assets: Record<string, string> = {};
  try {
    const manifest = await import("./client-manifest.ts");
    assets = manifest.assets;
  } catch {
    // Manifest not generated — fall back to filesystem serving
  }
